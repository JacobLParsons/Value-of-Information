...
for(n in 1:length)
{
    ...
    {       
        ...
        phat.logit = p.logit + chain$theta[n] + chain$delta[n,] + chain$gamma[n,list] + rnorm(27, mean = 0, sd = sqrt(chain$sigmaESq[n]/1000) )
        phat.logit2 = p.logit2 + chain$theta[n] + chain$delta[n,] + chain$gamma[n,list] + rnorm(27, mean = 0, sd = sqrt(chain$sigmaESq[n]/1000) )
           
    }

    gamma =  rnorm(n = 1, mean=0, sd = sqrt(chain$sigmaGammaSq[n]) )
    phat.logit.newlist = p.logit.newlist + chain$theta[n] + chain$delta[n,] + gamma + rnorm(27, mean = 0, sd = sqrt(chain$sigmaESq[n]/1000))
    phat.logit.newlist2 = p.logit.newlist2 + chain$theta[n] + chain$delta[n,] + gamma + rnorm(27, mean = 0, sd = sqrt(chain$sigmaESq[n]/1000))
...
save(list = c("table1", "table2", "table3"), file = "./tables3.RData")
