...
size = c(36,30,71,65,107,100,69,65,28,22,93,87,55,50)

EVSI = array(NA, dim = c(7, length(targets)))
simulatedRVSI = array(NA, dim = c(7, length(targets), 15000))
RVSI = array(NA, dim = c(7, length(targets)))
EVOIR = array(NA, dim = c(7, length(targets)))
...
for(i in 1:length(targets))
{
    ...
    for(list in 1:7)
    {
        ...
        if(target > 30)
            prediction = prediction - table2[,30]
        
        EVSI[list,i] = var(prediction)
        simulatedRVSI[list,i,] = prediction

        load(paste("./",list,"/all.RData", sep = ""))

        estimates.rem = rep(NA, 11)
        
        estimates.rem[1] = mean(chain$theta)
        estimates.rem[2] = mean(chain$mu)
        estimates.rem[3] = mean(digamma(chain$alpha0) - digamma(chain$beta0))
        estimates.rem[4] = mean(chain$yearTrend[,2])
        estimates.rem[5] = estimates.rem[4] + mean(chain$yearTrend[,3])
        estimates.rem[6] = estimates.rem[5] + mean(chain$yearTrend[,4])
        estimates.rem[7] = estimates.rem[6] + mean(chain$yearTrend[,5])
        estimates.rem[8] = estimates.rem[7] + mean(chain$yearTrend[,6])
        estimates.rem[9] = estimates.rem[8] + mean(chain$yearTrend[,7])
        estimates.rem[10] = estimates.rem[9] + mean(chain$yearTrend[,8])
        estimates.rem[11] = estimates.rem[10] + mean(chain$yearTrend[,9])

        RVSI[list,i] = (estimates.rem[i] - estimates[i])^2  
